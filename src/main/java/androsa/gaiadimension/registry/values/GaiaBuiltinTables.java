...
public class GaiaBuiltinTables {
    ...
    public static final Map<SapperVariant, ResourceKey<LootTable>> SAPPER_TABLES = Util.make(
            new EnumMap<>(SapperVariant.class), map -> {
                for (SapperVariant variant : SapperVariant.values()) {
                    map.put(variant, makeTable("entities/" + variant.getVariant() + "_sapper"));
                }
            }
    );
