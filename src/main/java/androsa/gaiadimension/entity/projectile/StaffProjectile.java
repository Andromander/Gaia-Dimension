...
public class StaffProjectile extends AbstractHurtingProjectile {
    ...
    private void bounceProjectile(HitResult result) {
        ...
        if (result.getType() == HitResult.Type.ENTITY) {
            ...
            movement = new Vec3(movement.toVector3f().reflect(entityresult.getEntity().getDirection().step()));
        } else if (result.getType() == HitResult.Type.BLOCK) {
            BlockHitResult blockresult = (BlockHitResult) result;

            movement = new Vec3(movement.toVector3f().reflect(blockresult.getDirection().step()));
        }
    ...
    private void splitProjectile() {
        for (int i = 0; i < 5; i++) {
            StaffProjectile split = new StaffProjectile(this.level(), this.getX(), this.getY() + 0.15F, this.getZ());
            split.setElement(this.getElement());
            split.setBehavior(GaiaStaffItem.Behavior.BASIC);
            split.setStat(this.getStat());

            float angle = 72.0F * i;
            Vec3 vec3 = new Vec3(1.0F, 0.0F, 0.0F).yRot((float)Math.toRadians(angle));
            split.shoot(vec3.x(), vec3.y(), vec3.z(), 1.0F, 1.0F);
