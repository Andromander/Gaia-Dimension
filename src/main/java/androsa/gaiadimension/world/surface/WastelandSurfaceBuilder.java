...
public class WastelandSurfaceBuilder<T extends SurfaceBuilderBaseConfiguration> extends SurfaceBuilder<T> {
    ...
    @Override
    public void apply(Random random, ChunkAccess chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int sealevel, int minlevel, long seed, T config) {
        this.genWastelandTerrain(random, chunkIn, biomeIn, x, z, startHeight, noise, defaultBlock, defaultFluid, config.getTopMaterial(), config.getUnderMaterial(), config.getUnderwaterMaterial(), sealevel, minlevel);
    }

    public final void genWastelandTerrain(Random rand, ChunkAccess chunk, Biome biome, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, BlockState top, BlockState middle, BlockState bottom, int sealevel, int minlevel) {
                ...
                if (state.isAir()) {
                    ...
                    if (!flag) {
                        ...
                        if (y >= sealevel) {
                            ...
                            newstate = ModBlocks.wasteland_stone.get().defaultBlockState();
