...
public class WastelandSurfaceBuilder<T extends SurfaceBuilderConfig> extends SurfaceBuilder<T> {
    ...
    public void apply(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int sealevel, long seed, T config) {
        this.genWastelandTerrain(random, chunkIn, biomeIn, x, z, startHeight, noise, defaultBlock, defaultFluid, config.getTopMaterial(), config.getUnderMaterial(), config.getUnderwaterMaterial(), sealevel);
    }
    ...
    public final void genWastelandTerrain(Random rand, IChunk chunk, Biome biome, int x, int z, int startHeight, double noiseVal, BlockState defaultBlock, BlockState defaultFluid, BlockState top, BlockState middle, BlockState bottom, int sealevel) {
        ...
        for (int posY = startHeight; posY >= 0; --posY) {
            ...
            if (blockstate2.isAir()) {
                ...
                if (posY > sealevel + 10) {
                    chunk.setBlockState(blockpos$mutableblockpos, ModBlocks.wasteland_stone.get().defaultBlockState(), false);
                }
                if (j == -1) {
                    if (k <= 0) {
