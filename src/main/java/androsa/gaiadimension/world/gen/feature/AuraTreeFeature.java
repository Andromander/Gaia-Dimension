    ...
    public boolean generate(WorldGenLevel worldIn, Random rand, BlockPos position, BiConsumer<BlockPos, BlockState> logPos, BiConsumer<BlockPos, BlockState> leavesPos, T config) {
        ...
        if (position.getY() >= 1 && position.getY() + baseHeight + 1 <= 256) {
            ...
            if (!isValidBonemealTarget) {
                ...
                this.setBlock(worldIn, position.below(), ModBlocks.light_soil.get().defaultBlockState());
                ...
                for (int base = 0; base < baseHeight; ++base) {
                    ...
                    if (base > baseHeight / 2) {
                        if (isAirOrLeaves(worldIn, blockpos)) {
                            this.setLog(worldIn, rand, new BlockPos(posX - offset, i2, posZ), logPos, config);
                            this.setLog(worldIn, rand, new BlockPos(posX + offset, i2, posZ), logPos, config);
                            this.setLog(worldIn, rand, new BlockPos(posX, i2, posZ - offset), logPos, config);
                            this.setLog(worldIn, rand, new BlockPos(posX, i2, posZ + offset), logPos, config);
                            if (base % 2 == 0) {
                                offset += 1;
                            }
                ...
                for (int j3 = -1; j3 <= 1; ++j3) {
                    for (int i4 = -1; i4 <= 1; ++i4) {
                        if (Math.abs(j3) != 1 || Math.abs(i4) != 1){
                            this.setLeaves(worldIn, rand, blockpos2.offset(j3 - 3, 0, i4), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos2.offset(j3 + 3, 0, i4), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos2.offset(j3, 0, i4 - 3), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos2.offset(j3, 0, i4 + 3), leavesPos, config);
                        }
                    }
                }
                ...
                for (int j3 = -2; j3 <= 2; ++j3) {
                    for (int i4 = -2; i4 <= 2; ++i4) {
                        if (Math.abs(j3) != 2 || Math.abs(i4) != 2){
                            this.setLeaves(worldIn, rand, blockpos3.offset(j3 - 3, 0, i4), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos3.offset(j3 + 3, 0, i4), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos3.offset(j3, 0, i4 - 3), leavesPos, config);
                            this.setLeaves(worldIn, rand, blockpos3.offset(j3, 0, i4 + 3), leavesPos, config);
                        }
                    }
                }

                this.setLeaves(worldIn, rand, blockpos3.above(), leavesPos, config);
