...
public class CoatedCavesWorldCarver<T extends ProbabilityConfig> extends WorldCarver<T> {
    ...
    public CoatedCavesWorldCarver(Codec<T> config, int height) {
        ...
        carvableFluids = ImmutableSet.of(ModFluids.mineral_water_still.get());
    }
    ...
    protected void carveCave(IChunk chunkIn, Function<BlockPos, Biome> biomePos, long seed, int seaLevel, int chunkX, int chunkZ, double x, double y, double z, float radius, double diameter, BitSet mask) {
        double d0 = 1.5D + (double)(MathHelper.sin(((float)Math.PI / 2F)) * radius);
        double d1 = d0 * diameter;
        this.func_227208_a_(chunkIn, biomePos, seed, seaLevel, chunkX, chunkZ, x + 1.0D, y, z, d0, d1, mask);
    }

    protected void carveTunnels(IChunk chunkIn, Function<BlockPos, Biome> biomePos, long seed, int seaLevel, int centerX, int centerZ, double chunkX, double chunkY, double chunkZ, float radius, float yaw, float pitch, int baseSize, int maxSize, double diameter, BitSet mask) {
        ...
        for (int j = baseSize; j < maxSize; ++j) {
            ...
            chunkX += (double)(MathHelper.cos(yaw) * f2);
            chunkY += (double)MathHelper.sin(pitch);
            chunkZ += (double)(MathHelper.sin(yaw) * f2);
            ...
            if (j == i && radius > 1.0F) {
                this.carveTunnels(chunkIn, biomePos, random.nextLong(), seaLevel, centerX, centerZ, chunkX, chunkY, chunkZ, random.nextFloat() * 0.5F + 0.5F, yaw - ((float)Math.PI / 2F), pitch / 3.0F, j, maxSize, 1.0D, mask);
                this.carveTunnels(chunkIn, biomePos, random.nextLong(), seaLevel, centerX, centerZ, chunkX, chunkY, chunkZ, random.nextFloat() * 0.5F + 0.5F, yaw + ((float)Math.PI / 2F), pitch / 3.0F, j, maxSize, 1.0D, mask);
                return;
            }
