    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + height + 1 <= 256) {
            ...
            if (!flag) {
                ...
                boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, down, EnumFacing.UP, ((GDAgateSapling)GDBlocks.fossilized_sapling));
                ...
                if (isSoil && position.getY() < worldIn.getHeight() - height - 1) {
                    ...
                    for (int widthSize = -3; widthSize <= 3; ++widthSize) {
                        for (int lengthSize = -3; lengthSize <= 3; ++lengthSize) {
                            if (Math.abs(widthSize) != 3 || Math.abs(lengthSize) != 3){
                                this.placeLeafAt(worldIn, blockpos2.add(widthSize, 0, lengthSize));
                            }
                        }
                    }
