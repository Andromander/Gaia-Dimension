...
public class GaiaGBAULang extends GaiaEnglishLang {
    public GaiaGBAULang(PackOutput output, String locale) {
        super(output, locale);
    }
    ...
    public void genSpecific() {
        addTree(ModBlocks.fossilized_sapling, ModBlocks.fossilized_leaves,
                ...
                ModBlocks.potted_fossilized_sapling, "Fossilised");
        addDeathMessage("malachiteBlast", "%1$s was vapourised", "%1$s was vapourised before %2$s");
        addBiome(GaiaBiomes.smoldering_bog, "Smouldering Bog");
    }

    @Override
    public void addStaffHeadings() {
        super.addStaffHeadings();
        addStaffTip("desc.behavior", "Behaviour");
    }

    @Override
    public void addKitTooltips() {
        super.addKitTooltips();
        addKitTip("replace", "Use this to make a part a different Mookaite colour");
        addKitTip("color", "This sets the colour:");
    }
}
